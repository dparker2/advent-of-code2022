...
expand_path(EndXY, [XY|Path], Visited, List) :- length([XY|Path], L),
  findall(
    P-[Next,XY|Path],
...
search(Heap0, EndXY, Visited, Path) :- get_from_heap(Heap0, _, MinPath, Heap),
  % writeln(MinPath),
  ( nth0(0, MinPath, EndXY)
    -> Path = MinPath
    ; (
      expand_path(EndXY, MinPath, Visited, PathList),
